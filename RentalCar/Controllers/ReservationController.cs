        ...
        public async Task<IActionResult> Edit(Reservation reservationViewModel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (reservation.ReservedTo.ToUniversalTime().CompareTo(DateTime.UtcNow) > 0)
                {
                    ...
                    reservation.NumberOfDays = (reservationViewModel.ReservedTo.ToUniversalTime().Date - reservation.ReservedFrom.ToUniversalTime().Date).Days > 0 ? (reservationViewModel.ReservedTo.ToUniversalTime().Date - reservation.ReservedFrom.ToUniversalTime().Date).Days : 1;
                }
